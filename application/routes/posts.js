...
const { makeThumbnail, getPostById, getCommentsForPostById, getRecentPosts } = require("../middleware/posts");
...
router.get("/:id(\\d+)", getPostById, getCommentsForPostById,function(req,res){
    res.render('viewpost',{ title: 'viewpost',css:["style.css"]});
});
...
router.get("/search", async function (req,res,next){
    ...
    try{
        ...
        if(rows && rows.length == 0){
            req.flash("error",`No posts found, here are recent posts though`);
            return res.redirect('/');
        }else{
